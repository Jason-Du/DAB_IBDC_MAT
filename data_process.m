...
% %     % filter value< mean-2sigma  STATE
...
%         % % % 計算出Rtheta
        ...
        if(read_i==1)
            R_nominal=median(R_array); % R nominal 定義
        end
        Zk_array=R_nominal-R_array;%  Degration resistance =Nominal resistance - Operation resistance
        R_theta_array=(Zk_array/R_nominal)*100;
%         % % % 計算出Rtheta filt
        R_array_filt=Vds_med_array./ID_med_array;% R mid
        if(read_i==1)
            R_nominal=median(R_array_filt); % R nominal 定義
        end
        Zk_array_filt=R_nominal-R_array_filt;
        R_theta_array_filt=(Zk_array_filt/R_nominal)*100;
...
% % % %PLOT delta R
f2 = figure;
x1=linspace(0,duration_t,ON_state_data_numbers_t);
...
% % f4 = figure;
subplot(2,1,2);
    plot(x2, R_theta_array_filt_t,'-b');
    title('delta R filt')
    legend('delta R filt')
